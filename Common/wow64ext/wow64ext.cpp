...
static BOOL g_initialize_modules = FALSE;
...
int _lwcsicmp(const wchar_t *string1, const wchar_t *string2)
{
    wchar_t c1;
    wchar_t c2;
    int i = 0;
...
DWORD64 __cdecl GetModuleHandle64(wchar_t* lpModuleName)
{
    ...
    do
    {
        ...
        wchar_t* tempBuf = (wchar_t*)lmalloc(head.BaseDllName.MaximumLength);
        if (NULL == tempBuf)
            return 0;
        WATCH(tempBuf);
        getMem64(tempBuf, head.BaseDllName.Buffer, head.BaseDllName.MaximumLength);

        if (0 == _lwcsicmp(lpModuleName, tempBuf))
            return head.DllBase;
    }
...
DWORD64 getLdrGetProcedureAddress()
{
    ...
    DWORD* rvaTable = (DWORD*)lmalloc(sizeof(DWORD)*ied.NumberOfFunctions);
    if (NULL == rvaTable)
        return 0;
    WATCH(rvaTable);
    getMem64(rvaTable, modBase + ied.AddressOfFunctions, sizeof(DWORD)*ied.NumberOfFunctions);
    
    WORD* ordTable = (WORD*)lmalloc(sizeof(WORD)*ied.NumberOfFunctions);
    if (NULL == ordTable)
        return 0;
    WATCH(ordTable);
    getMem64(ordTable, modBase + ied.AddressOfNameOrdinals, sizeof(WORD)*ied.NumberOfFunctions);

    DWORD* nameTable = (DWORD*)lmalloc(sizeof(DWORD)*ied.NumberOfNames);
    if (NULL == nameTable)
        return 0;
    WATCH(nameTable);
